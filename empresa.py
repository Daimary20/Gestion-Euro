...
import pandas as pd # Necesario para el gráfico
...
if not st.session_state['autenticado']:
            ...
            if cod == CODIGO_REGISTRO_ADMIN:
                if user_alias and ced and cla:
                    supabase.table("usuarios").insert({
                        "usuario": user_alias, "cedula": ced, "correo": cor, "clave": cla, "nombre_completo": f"{nom_real} - {car}"
                    }).execute()
    ...
    if menu == "📋 Historial":
        ...
        if res.data:
            ...
            with st.expander("📊 Ver Estadísticas de Técnicos"):
                if datos:
                    df = pd.DataFrame(datos)
                    st.bar_chart(df['tecnico'].value_counts())
                else: st.write("No hay datos para mostrar gráficos.")
            ...
            for i in datos:
                ...
                with st.expander(f"{color} {i['fecha']} | {i['equipo']} - {i.get('area', 'N/A')}"):
                    ...
                    if i.get('comentario_supervisor'): st.info(f"🗨️ {i['comentario_supervisor']}")
                    if i['url_multimedia']:
                    ...
                    if es_admin:
                        st.divider()
                        obs = st.text_input("Comentario", key=f"o_{i['fecha']}_{i['equipo']}")
                        ca, cb, cc = st.columns(3)
                        # Usamos campos únicos para las llaves
                        if ca.button("✅ Confirmar", key=f"ok_{i['fecha']}_{i['equipo']}"):
                            supabase.table("reportes_euro").update({"estado": "Confirmado", "comentario_supervisor": f"{obs} (Rev: {u_actual})"}).eq("fecha", i['fecha']).eq("equipo", i['equipo']).execute()
                            st.rerun()
                        if cb.button("❌ Observar", key=f"no_{i['fecha']}_{i['equipo']}"):
                            supabase.table("reportes_euro").update({"estado": "Observado", "comentario_supervisor": f"{obs} (Rev: {u_actual})"}).eq("fecha", i['fecha']).eq("equipo", i['equipo']).execute()
                            st.rerun()
                        if cc.checkbox("Eliminar", key=f"del_chk_{i['fecha']}_{i['equipo']}"):
                            if st.button("Confirmar", key=f"del_btn_{i['fecha']}_{i['equipo']}"):
                                supabase.table("reportes_euro").delete().eq("fecha", i['fecha']).eq("equipo", i['equipo']).execute()
                                st.rerun()
    ...
    if menu == "👥 Personal":
        ...
        for us in u_res.data:
            ...
            if us['usuario'] != u_actual:
                # Usamos el campo 'usuario' como llave única para evitar el KeyError 'id'
                if c_b.button("Eliminar", key=f"del_u_{us['usuario']}"):
