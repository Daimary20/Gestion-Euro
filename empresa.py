...
def generar_pdf(lista_reportes):
    ...
    pdf.cell(35, 8, "Área/Ubicación", 1, 0, "C", True) # Columna de Área
    ...
    for r in lista_reportes:
        ...
        pdf.cell(35, 8, str(r.get('area', ''))[:20], 1) # Datos del Área
...
if not st.session_state['autenticado']:
    ...
    # ACCESO ADMINISTRATIVO: Daimary Salas tiene permiso total
    ...
    if menu == "➕ Actividad":
        ...
        with st.form("f_act"):
            ...
            ar = st.text_input("Área a la que pertenece") # Campo solicitado
            ...
            for i in datos:
                with st.expander(f"{i['fecha']} | {i['equipo']} ({i.get('area', 'Sin Área')})"):
                    ...
                    if es_admin:
                        ...
                        c1, c2 = st.columns(2)
                        # Firma con el nombre del revisor
    ...
    if menu == "👥 Personal":
        ...
        for us in u_res.data:
            ...
            # Daimary Salas puede eliminar cualquier usuario menos a ella misma
            if us['usuario'] != u_actual:
                if c_b.button("Eliminar", key=f"del_{us.get('id', us['usuario'])}"):
